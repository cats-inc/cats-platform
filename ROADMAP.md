...
- [x] Record official app source ownership in `cats-apps` and initial coordinated
      Desktop distribution in ADR-114; name the first utility Usage.
- [x] Finish managed versioned-package installation and a real installed renderer
      loader with a permission-checked public SDK.
- [x] Expose a scoped, read-only runtime telemetry view for Usage.
- [x] Bundle pinned, prebuilt app artifacts into Desktop staging and verify isolated offline launch,
...
- [x] Make unsupported provider/model reasoning-effort combinations explicit in
      ...
      Completed: provider selector view state now reports unsupported persisted
      control values for the selected runtime catalog entry before reconciliation
      sanitizes them, and the composer renders that warning near the controls.
