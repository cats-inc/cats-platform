...
| Guide Cat and Participant Generalization | In Progress | The documentation freeze is landed and the first setup-contract slice is now in-tree: `Guide Cat` persists as optional suite-level state, setup can complete with or without it, and setup no longer silently copies Guide Cat runtime settings onto the global orchestrator; starter-suggestion and participant-generalization follow-ons still remain |
...
- [x] Guide Cat exists as a suite-level reusable helper rather than only as
      Boss Cat bootstrap
