...
| Documentation | In Progress | Top-level docs now reflect the three-step setup wizard, Cat-private in-place lane entry, Telegram inbox MVP, runtime skill/guardrail seams, shared-core projection boundaries, the operator-loop chat surfaces, the companion-box sidecar/session-hydration contract, the Cats-owned canonical memory/retrieval substrate with promotion rules, source convergence, Team 5 flush payloads, the orchestrator execution-loop/MCP seam, the first Electron desktop-host slice, and the polling-first Telegram follow-on direction; broader launch-track docs still remain |
