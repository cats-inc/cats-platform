...
| Complete the unified conversation-turn-lane engine migration | [x] | Chat now runs on the canonical `Container -> Conversation -> Turn -> Lane -> Segment -> Session` engine, including lane-first live projection, canonical replay/repair/recovery, decomposed runtime-session wake helpers, concurrent attach barriers, transport-binding continuity, and typed workspace busy-state contracts |
...
| Keep ordinary `Recents` entry compatible with default-composer mode | [x] | Normal `/new` chats no longer have to materialize as visible chat-channel threads before the operator decides to add a Cat |
...
- [x] Chat live, refresh, reconnect, replay, and repair all converge on the
      same canonical conversation/turn/lane/segment/session engine contract
...
| Task | Status | Notes |
|------|--------|-------|
| Add work dashboard and inbox surfaces | [x] | `/api/work`, `/api/work/tasks`, and the Work renderer now expose `/work/war-room`, `/work/tasks`, and `/work/tasks/:taskId` above core-owned operator inbox, control-plane, recovery, and timeline reads, with War Room operator/control/recovery cards also carrying shared task-context quick links back into briefing threads and cat assignee lanes when that context exists |
...
- [x] `Cats Work` can render useful work views without inventing a separate schema
- [ ] Chat and Work share the same actor, conversation, approval, and owner-profile contracts
- [x] Work surfaces stay above the same runtime boundary used by Chat

---

### WP-6: Guide Cat and Participant Generalization
...
| Task | Status | Notes |
|------|--------|-------|
| Freeze `Guide Cat` terminology across setup and planning docs | [x] | `Guide Cat` is now the preferred term over `assistant` for the optional first helper |
...
- [ ] Entry surfaces can use Guide Cat suggestions while retaining deterministic
      fallback ideas
- [ ] Chat and shared-core docs no longer require Cat-only language to explain
      conversation topology and routing strategy
...
- runtime targeting now also keeps same-participant prompt history visible
  across preseeded lane churn, so stable default chats and anti-ping-pong
...
- default-chat bootstrap suppression now prefers canonical `laneId` over reused
...
- those same core recovery routes now also support `latestReplayTrigger` plus
...
- validation coverage now includes `/api/work`, `/api/code`, and the current
  platform route map
- `/api/work/tasks/{taskId}` and `/api/code/tasks/{taskId}` now both mirror the
  same task-scoped shared `controlPlane` / `recovery` read models that back
...
- runtime targeting session context and derived memory layers now also resolve
  `containerId` / `conversationId` through that shared helper, and default-chat
