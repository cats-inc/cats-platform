...
- `src/core/taskControlPlane.ts` plus `GET /api/core/control-plane/tasks` and
  ...
  delivery intent, and operator-attention classification on top of existing
  core write seams, so future operator inbox or recovery tooling can consume a
  single task-level control-plane view instead of reassembling recovery,
  inspection, and records responses client-side
