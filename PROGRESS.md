...
| Renderer Shell | Completed | React/Vite now runs as a route-driven suite shell above the Node server, with Chat as the active product surface, a first real Work dashboard route, a Code placeholder route, and top-level suite ownership split under `src/app/*` and `src/products/*` |
...
| Add project/work-item views on top of shared contracts | [ ] | Shared-core project/work-item records exist, but Work still needs richer product-specific board/project views |
