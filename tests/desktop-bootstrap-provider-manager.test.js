...
async function page(t, { observations = [], selected = [codex, ollama], recovery = false } = {}) {
  ...
  const snapshot = { phase: recovery ? 'failed' : 'ready_for_setup', summary: 'Setup fixture',
    ...
    prerequisites: { providerSelection: selection, providerCatalog: state.runtime.universe,
      cliInventory: { source: 'runtime', installed: [], total: 0, scannedAt: null,
  ...
  const actions = []; const oldActions = [];
  ...
  return { root, state, actions, oldActions, bridge: dom.window.catsDesktopHost,
