...
test('normal renderer loads and product changes sync tray data without re-running setup', async () => {
  ...
  try {
    ...
    await act(async () => { root.render(<Probe value={envelope} />); });
    assert.deepEqual(updates, [envelope]);
    await act(async () => { root.render(<Probe value={{ ...envelope, ownerDisplayName: 'Changed' }} />); });
    ...
    await act(async () => { root.render(<Probe value={disabled} />); });
