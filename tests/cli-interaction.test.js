...
import { browserCommand, browserUrl, startCliInteraction } from '../build/server/shared/cliInteraction.js';
...
function fixture(overrides = {}) {
  const input = overrides.input ?? new Terminal();
  const output = overrides.output ?? new Terminal();
  const opened = [];
  const stopped = [];
  let text = '';
