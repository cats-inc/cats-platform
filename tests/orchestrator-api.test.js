...
test('POST /api/orchestrator/plan projects external tracker binding intent', async () => {
  await withServer(createRuntimeStub(), async (baseUrl) => {
    const created = await createChannel(baseUrl, {
      roomMode: 'direct_message',
      cats: [
    ...
    const response = await fetch(`${baseUrl}/api/orchestrator/plan`, {
      ...
      body: JSON.stringify({
        channelId,
        body: 'Boss Cat link work-item-alpha to https://redmine.example.com/issues/987',
      }),
    });
...
test('POST /api/orchestrator/dispatch applies provider external issue import decisions', async () => {
  ...
  await withServer(runtimeClient, async (baseUrl) => {
    ...
    chat = setChannelCatLease(
      ...
      {
        status: 'ready',
        sessionId: 'session-work-external-import',
        laneId: 'lane-work-external-import',
      },
      new Date('2026-05-13T00:00:00.000Z'),
    );
    await chatStore.write(chat);
    ...
    const updatedCore = await chatStore.readCore();
    const workItem = updatedCore.workItems.find((candidate) =>
      candidate.title === 'Import GitHub issue into Cats Work');
    assert.ok(workItem);
    assert.equal(workItem.status, 'planned');
    assert.equal(workItem.summary, 'Issue body from GitHub.');
    const bindings = Array.isArray(workItem.metadata.externalWorkBindings?.bindings)
      ? workItem.metadata.externalWorkBindings.bindings
      : [];
    assert.equal(bindings.length, 1);
    ...
    assert.equal(metadata?.created, true);
    assert.equal(metadata?.linked, true);
  }, chatStore, {
    work: {
      externalIssueImport: {
        github: {
