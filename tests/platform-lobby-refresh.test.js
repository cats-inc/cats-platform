...
test('PlatformApp refetches the full platform envelope when navigation enters the lobby', async () => {
  ...
  const routePathsSource = await readFile(
    new URL('../src/shared/platformRoutePaths.ts', import.meta.url),
    'utf8',
  );

  assert.match(routePathsSource, /export function isLobbyPath\(pathname: string\)/u);
  assert.match(source, /from '\.\.\/\.\.\/shared\/platformRoutePaths\.js'/u);
