#!/bin/bash
#
# Configure cats + Tailscale Funnel auto-start on macOS.
#
...
RUNNER_SCRIPT="$APP_SUPPORT_DIR/start-cats-tailscale-funnel.sh"
PLIST_FILE="$LAUNCH_AGENTS_DIR/io.cats.tailscale-funnel.plist"
...
<plist version="1.0">
  <dict>
    <key>Label</key>
    <string>io.cats.tailscale-funnel</string>
...
if [[ "$COMMAND" == 'remove' ]]; then
  ...
  stop_managed_server
  unload_launch_agent
  rm -f "$RUNNER_SCRIPT" "$PLIST_FILE" "$RUNNER_LOG" "$STATE_DIR/cats-server.out" "$STATE_DIR/cats-server.err"
