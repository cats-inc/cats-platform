...
assert_file "${RESOURCES_ROOT}/app-sidecar/dist-server/index.js" 'bundled cats server entry'
assert_file "${RESOURCES_ROOT}/app-sidecar/dist/index.html" 'bundled cats renderer build'
...
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/setup-node-global-prefix.sh" 'bundled Linux npm prefix helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/install-node-cli-tools.sh" 'bundled Linux node CLI pack helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/install-claude-code.sh" 'bundled Linux Claude Code installer helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/install-cursor-agent.sh" 'bundled Linux Cursor Agent installer helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/install-goose.sh" 'bundled Linux Goose installer helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/install-junie.sh" 'bundled Linux Junie installer helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/install-kiro-cli.sh" 'bundled Linux Kiro installer helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/linux/check-installation.sh" 'bundled Linux readiness audit helper'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/shared/unix-provider-cli-common.sh" 'bundled shared Unix provider helper library'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/shared/unix-node-cli-common.sh" 'bundled shared Unix npm helper library'
assert_file "${RESOURCES_ROOT}/desktop-host/setup-assets/manifest.json" 'bundled setup-assets manifest'
