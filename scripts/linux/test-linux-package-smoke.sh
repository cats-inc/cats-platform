...
assert_file "${RESOURCES_ROOT}/app-sidecar/config/provider-capability-bootstrap.yaml.example" 'bundled cats capability bootstrap example'
...
assert_file "${RESOURCES_ROOT}/desktop/setup-assets/linux/install-meta-cli.sh" 'bundled Linux Meta Muse installer helper'
...
assert(linuxTarget.artifacts.some((artifact) => artifact.id === 'linux-muse-native-installer-script'), 'Linux target includes the bundled Meta Muse installer asset');
