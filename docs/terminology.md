...
| Participant | A canonical Interaction Core record (`ParticipantRecord`). One `Agent`'s membership inside one `Conversation`, carrying role, status, and execution-lease context. Do not conflate `Participant` (durable membership) with `Session` (ephemeral runtime attachment). See the full product-facing definition under **Product Terms**. |
| Transport binding | A canonical Interaction Core record (`TransportBindingRecord`). Maps one external transport thread (e.g. a Telegram DM) to a Cats entry path / conversation; lives in Interaction Core because it selects the `Conversation` that a transport ingress lands in. See the full product-facing definition under **Product Terms**. |
