...
| Task product binding | The derived Work Graph task projection label: `work`, `code`, `chat`, or `unbound`. `work` comes only from a structural `WorkItem.taskId` bridge; Code / Chat / unbound come from artifacts, planning metadata, or conversation fallback when no WorkItem owns the task. |
...
| Chat Task | A `Task` that Chat deliberately materializes from an already-active Chat conversation, such as a user-created follow-up / action item or assistant-generated chat-side plan. It is not created by default for `+New chat`, not created by Chat -> Code draft activation, and not a separate record type. It does not require a Project or WorkItem unless explicitly linked into Work. Work Graph surfaces should show it with `productBinding = chat` and place it under `No project` when no Planning lineage exists. |
