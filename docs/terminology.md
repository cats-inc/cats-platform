...
| Outcome | The durable success/failure conclusion for a `Task` or `Run` (`CoreOrchestrationOutcomeRecord`). Execution evidence — usually run-bound, but `runId` is nullable and an outcome may be anchored only on a `Task` and/or `Conversation`. Not a peer of `Run`. Legacy terms `Execution Result` / `Result` resolve here. |
| Checkpoint | A mid-execution state snapshot (`CoreCheckpointRecord`). Execution evidence — usually run-bound, but `runId` is nullable and a checkpoint may be anchored only on a `Task` and/or `Conversation`. Not a peer of `Run`. |
| Trace | The execution event log (`CoreTraceRecord`) produced during Task / Run execution — `note`, `status`, `dispatch`, `approval`, `checkpoint`, `outcome`, `error` kinds. Execution evidence — usually run-bound, but `runId` is nullable and a trace may be anchored only on a `Task` and/or `Conversation`. A distinct record from `Activity`. |
