...
| New Code | The one-person `Cats Code` entry preset. Task-first conversation: it creates one primary coding `Conversation` and one primary Code-owned `Task`, but does not force a `Project`, `WorkItem`, or `Run` at creation time. |
| Team Code | The shared-room `Cats Code` entry preset that creates one multi-participant coding conversation with workflow policy. Code presets remain Code-owned and task-first, but runs are still created only when execution attempts start. |
...
| Task product binding | The Work Graph task projection label: `work`, `code`, `chat`, or `unbound`. It tells Work surfaces which product owns / executes a shared Core `Task` without turning Code or Chat tasks into managed-work records. |
| No project task bucket | The honest Work UI home for tasks with no Project / WorkItem lineage. It may be grouped or labeled by task product binding, especially `code`, `chat`, or `unbound`, and must not silently create a fallback project. |
...
| Chat Task | A `Task` that Chat owns through chat-side planning or conversation provenance. It is not a separate record type, and it does not require a Project or WorkItem unless explicitly promoted into Work. Work Graph surfaces should show it with `productBinding = chat` and place it under `No project` when no Planning lineage exists. |
...
| Run | One concrete execution attempt for a `Task` or `Mission` (one coder session, tool batch, build, retry, takeover). Self-nestable via `parentRunId`. A run is not the durable objective and is not an entry-create record; one task accumulates many runs over time. |
