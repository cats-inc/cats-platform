...
| Task product binding | The derived Work Graph task projection label: `work`, `code`, `chat`, or `unbound`. Precedence (encoded by `resolveTaskProductBinding` in `src/products/work/api/projectionSupport.ts`): (1) `WorkItem.taskId` bridge → `work`; (2) `build` / `preview` `Artifact` on the task → `code`; (3) explicit planning provenance (`planning.productHint` or `planning.transfer.suggestedProduct`) → that product, with explicit `'work'` without a WorkItem demoted to `unbound` (incomplete-Work-claim diagnostic); (4) `Conversation.kind === 'code_thread'` legacy fallback → `code`. Chat-* / `work_thread` / `direct_message` / `external_transport` / `private_escalation` conversation kinds alone do **not** bind: chat requires explicit chat planning provenance per the deliberate-only Chat Task producer rule, and work requires the structural `WorkItem.taskId` bridge. The projection exposes only the *current* binding; if a Code-origin task is later linked through `WorkItem.taskId`, the projection emits `work`, and the Code lineage stays on the underlying `CoreTaskRecord.metadata.planning` for diagnostic / audit but is not surfaced as a separate field on `WorkGraphObjectSummary`. |
...
| Execution dispatcher / runtime bridge | The platform seam that admits an executable attempt for a `Task` or `Mission` and is therefore the unique boundary that emits a `Run`. Concretely two paths: (1) **assignment-driven dispatch** — `applyTaskAssignmentLifecycle` (in `src/core/taskLifecycle.ts`) calling `RuntimeClient.createWakeup` (in `src/runtime/client.ts`) when an assignment becomes dispatchable, with payload formed by `buildTaskRuntimeExecutionRequest` (in `src/shared/taskExecutionBridge.ts`); (2) **interactive submit** — per-product `runtime-session/taskExecution` modules (`src/products/<product>/state/runtime-session/taskExecution.ts` plus `src/products/<product>/state/taskExecutionRequest.ts`) calling `RuntimeClient.sendMessage`. *Admission* is the act of either queuing a wakeup request or dispatching a runtime turn payload — it is not the act of opening an entry shell, persisting a `Task` row, or rendering an unsent first turn as transcript text. The Code first-send case typically admits in the same submit flow as the entry, so `+New code` Run-laziness is on the order of seconds; Work usually waits for a separate supervised execute / continue action, so its laziness window is much longer. |
...
| Artifact | A durable output (`CoreArtifactRecord`) whose `kind` is one of `document`, `report`, `build`, `preview`, `attachment`, `transcript_export`, `dataset`. A materialization record rather than strictly a Run by-product: `build` and `preview` typically carry a `runId`, but `document` / `report` / `attachment` / `transcript_export` / `dataset` often have no `runId` and attach directly to a project, work item, or conversation. All back-references (`projectId` / `workItemId` / `conversationId` / `taskId` / `runId`) are nullable. |
