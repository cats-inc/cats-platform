...
- **Materialization records** (durable state produced around the Planning / Execution / Interaction graph, not peer entities of `Run`): `Artifact`, `Outcome`, `Checkpoint`, `Trace`, `Activity`. Some rows are direct run by-products (e.g. `Outcome`, `Checkpoint`, `build` / `preview` `Artifact`); others have no `Run` at all (e.g. operator-authored `Activity` with kinds `note` / `operator_action` / `work_item_updated`, or `Artifact` of kind `document` / `report` / `attachment` / `transcript_export` / `dataset`). `Activity` is an independent operator-feed record with its own `/api/core/activities` surface — it is **not** a projection over `Trace`, and consumers must read it directly.
