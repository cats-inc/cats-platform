...
`cats` will call `cats-runtime` for runtime status and future session
operations. It will not import `agent-fleet` internals or expose
backend-specific routes in its public contract.

## Consequences

- `cats` stays aligned with the monorepo migration direction
...
`cats` can start with a thin health and shell contract immediately.
