...
8. Skills sync (`Sync-AgentSkills.ps1`, `sync-agent-skills.sh` on macOS and Linux) gains no `antigravity` `--agent` value and no skills target. The probe this point made a precondition was run on 2026-08-28. Probing took two passes. The first seeded `<workspace>/.gemini/skills/` and `<workspace>/.antigravity/skills/` with codeword-bearing skills; neither appeared, and that was over-read as agy having no project-level skill discovery — a universal negative drawn from the two paths the stale documentation happened to name. The second pass, prompted by review, seeded `<workspace>/.agents/skills/probe-agents/SKILL.md` alongside a `.gemini/skills/` control: **agy reported the `.agents/skills/` skill by absolute path and again did not report the `.gemini/skills/` one.** The agy 1.1.22 binary carries seven `.agents/skills` literals, and Google's Antigravity documentation specifies `<workspace-root>/.agents/skills/` with a migration path off `.gemini/skills/`. So agy does discover project-level skills, at `.agents/skills/` — the same directory Codex already reads. This point's conclusion is unchanged but its reason is different: no `antigravity` `--agent` value or new target is needed because anything synced to `.agents/skills/` for Codex is already visible to agy. `.gemini/skills/` remains unread, so dropping it stays correct. An `--agent antigravity` alias pointing at the same directory would be a naming convenience, not a delivery mechanism. The `gemini` row this point proposed removing does not exist in this repository's helpers (see Context).
...
- ~~Skills tooling has an open question on `.antigravity/skills/` that cannot be resolved without probing `agy` behavior.~~ Resolved 2026-08-28 by the probe recorded in decision point 8: agy reads project-level skills from `.agents/skills/`, the directory Codex already uses, so no new sync target is required and the skills-tooling slice reduces to a documentation correction. No longer on the critical path.
