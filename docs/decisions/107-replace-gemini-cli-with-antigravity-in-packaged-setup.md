...
3. The desktop host packaging metadata (`packaging.ts`, `setupAssets.ts`) replaces the `windows/linux/macos-gemini-native-installer` asset ids and `Install-Gemini.{ps1,sh}` paths with `antigravity` equivalents. `setupAssets.ts` must not string-swap Antigravity into the existing npm-provider tuples, because those tuple-generated rows describe npm-global / repo-owned npm helper behavior. Antigravity gets explicit standalone provider-installer registrations whose notes describe user-scoped native binary install via environment-bootstrap.
...
7. The shared provider catalog (`src/shared/providerCatalogData.ts`, `src/shared/providerCatalogInstances.ts`) replaces the `gemini` provider family with `antigravity`. Antigravity catalog model values are populated only from explicit Phase 0 evidence. Official product documentation may populate display-name values, but raw `agy` model-id strings require a CLI model-list command, a documented config surface, or a smoke run proving the id is accepted. Existing Gemini model ids may move under `antigravity` only with that evidence.
