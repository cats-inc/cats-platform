...
> Use Paperclip as a product reference for `cats`, not as the runtime or
...
this document only if the platform explicitly chooses to resume a larger
...
`cats` already has an explicit architectural constraint from
...
`cats` will absorb Paperclip's strongest product concepts incrementally, but
it will not adopt Paperclip itself as the runtime or implementation base.

From this point on:

1. `cats` will evolve from a chat-first shell into a broader company
   control plane.
...
- `cats` can adopt a stronger operator-facing product model without losing
...
- **Cons**: Pulls `cats` toward Paperclip's server, data model, and runtime
