...
parser a stable version boundary. `scopeId` is the platform-host product data
scope UUID for the current durable Cats product data root — generated by the
cats-platform process that owns the data root (the desktop main process for
local-first installs), persisted next to that data root, and surfaced to all
renderer surfaces through the shared app-shell record. It is not an auth
account id, browser storage value, or workspace id.
