...
> Keep `cats-inc` as the owner of skill intent, profile binding, and policy
...
`cats-inc` now needs a stable answer for how `SKILL.md` should work across the
...
1. `cats-inc` could try to become a parallel skill package host
...
`cats-inc` and `cats-runtime` will split skill ownership by layer.

1. `cats-inc` owns product skill intent.
   - skill profiles
...
3. Execution-time `SKILL.md` packages should be hosted in `cats-runtime`, not
   directly mounted from `cats-inc`.
   - `cats-runtime` remains the runtime boundary and execution authority
   - `cats-inc` should request skills by stable runtime-facing identity, not by
     pushing arbitrary local package paths into sessions

4. `cats-inc` may still define product-authored skill profiles and curated
...
5. `Boss Cat` should use skills as capability packs, not as its source of
   authority.
   - routing and room policy stay in `cats-inc`
   - reusable know-how may live in runtime-hosted skills selected by
     `cats-inc`

6. If `cats-inc` later needs a skill authoring UX, that workflow should publish
...
- `cats-inc` can bind different Cats, room modes, and transport contexts to
...
- This ADR does not force `cats-inc` to build a full skill registry UI before
...
### Alternative 1: Let `cats-inc` host execution-ready `SKILL.md` packages
