...
- Skills sync tooling on all three OSes (`Sync-AgentSkills.ps1`, `sync-agent-skills.sh`) treated `gemini` as a valid `--agent` value and wrote to `.gemini/skills/`. Removed by `cc2d38de` under PLAN-100; verified 2026-08-28 that all three now accept only `claude` and `codex`. Requirements 16-17 below are therefore already satisfied.
...
- [x] Does `agy` expose a `.antigravity/skills/` directory or an equivalent skills mechanism? Answered from official docs, then confirmed by direct probe on 2026-08-28 against agy 1.1.20. A scratch workspace was seeded with `<workspace>/.gemini/skills/probe-gemini/SKILL.md` and `<workspace>/.antigravity/skills/probe-antigravity/SKILL.md`, each holding a unique codeword, and one print-mode turn asked agy to enumerate the skills in its context. Neither probe skill appeared. The conclusion holds: Sync-AgentSkills drops `gemini` without adding an `antigravity` row. The probe also corrects the observed roots, which the doc-derived answer had slightly wrong — agy loads from `~/.gemini/antigravity-cli/builtin/skills/<name>/SKILL.md` and `~/.gemini/config/plugins/<plugin>/skills/<name>/SKILL.md`, not from `~/.gemini/antigravity-cli/plugins`. Both were verified on disk.
