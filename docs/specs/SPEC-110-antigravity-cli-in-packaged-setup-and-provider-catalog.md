...
1. `scripts/windows/Install-Gemini.ps1` shall be replaced by `scripts/windows/Install-Antigravity.ps1`. The new wrapper shall keep the packaged helper action surface (`-CheckOnly`, `-Apply`, `-Upgrade`, `-Force`, `-Uninstall`, `-DryRun`, `-Json`, state hints). `-Apply` maps to default upstream install with no install-mode flag; `-Upgrade` maps to upstream `-Upgrade`; `-Force` maps to upstream `-Force`; packaged Windows calls always pass upstream `-NonInteractive`; `-CheckOnly`, `-DryRun`, `-Json`, and `-Uninstall` are wrapper-owned and shall not be delegated.
2. `scripts/macos/install-gemini.sh` shall be replaced by `scripts/macos/install-antigravity.sh`. The canonical host-facing contract is the same PowerShell-style flag set emitted by `desktop/host/setupBridge.ts`: `-CheckOnly`, `-Apply`, `-Upgrade`, `-Force`, `-Uninstall`, `-DryRun`, `-Json`. The shell wrapper shall not publish a separate bash-style lifecycle contract and shall parse flags with an explicit `case "$1" in -CheckOnly|-Apply|-Upgrade|-Force|-Uninstall|-DryRun|-Json)` loop rather than `getopts`. `-Apply` maps to default upstream install, `-Upgrade` maps to `-upgrade`, `-Force` maps to `-force`, and wrapper-owned check / dry-run / JSON / uninstall actions shall not be delegated.
...
- [x] What evidence source authoritatively lists or validates Antigravity model identifiers? The shared research note found official product documentation for selectable reasoning-model display names, but no live CLI/config/smoke evidence for raw `agy` model ids. Platform catalog values may use those display names; runtime dynamic model-id claims remain blocked until a later live probe proves them.
...
- [x] Is Antigravity's native binary download idempotently resumable from the packaged setup flow? Yes at the packaged-wrapper level: the wrappers re-check command/path presence before action and delegate refresh intent through the upstream force/upgrade flags instead of inventing a separate local reinstall path. `setupAssets.ts` should set `resumable: true` for the standalone Antigravity registrations.
