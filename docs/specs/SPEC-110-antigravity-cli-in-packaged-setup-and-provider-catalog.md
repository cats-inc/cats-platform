...
- [ ] Is Antigravity's native binary download idempotently resumable from the packaged setup flow? If not proven in PLAN-100 Phase 0, set `resumable: false` in the standalone `setupAssets.ts` registrations instead of inheriting the npm tuple default.
