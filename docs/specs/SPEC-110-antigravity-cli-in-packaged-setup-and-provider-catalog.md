...
- [x] Is Antigravity's native binary download idempotently resumable from the packaged setup flow? Yes at the packaged-wrapper level: the wrappers re-check command/path presence before action, and force/upgrade remove the binary before reinstalling. `setupAssets.ts` should set `resumable: true` for the standalone Antigravity registrations.
