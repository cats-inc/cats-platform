...
- **FR-17**: Extract Cats-owned static UI strings from these desktop renderer
  ...
  - shared design components that render user-visible text, aria labels, or
    tooltips
- **FR-18**: Extract user-visible renderer error/toast text where the message
  is Cats-owned and deterministic.
- **FR-19**: Known server-originated UI status/error messages that are rendered
  as product chrome should be converted to message keys or mapped to renderer
  messages. Unknown server errors may remain a raw fallback.
