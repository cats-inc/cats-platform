...
| [SPEC-076](./SPEC-076-per-entity-state-subscription-protocol.md) | Per-Entity State Subscription Protocol | Channel slice implemented; polymorphic proof pending | [PLAN-068](../plans/PLAN-068-per-entity-state-subscription-rollout.md), [PLAN-098](../plans/PLAN-098-polymorphic-entity-subscription-follow-up.md), [ADR-075](../decisions/075-adopt-push-based-per-entity-state-subscription.md) |
...
*Last updated: 2026-05-09 (SPEC-076 reconciled: channel entity-subscription slice implemented, polymorphic proof tracked by PLAN-098; SPEC-108 added: Cats Code Live Preview Substrate defines supervised process, port lease, logs, and Artifact Canvas preview-origin rules; SPEC-107 added: Preset-Neutral Product Intent Intake generalizes explicit `/chat` / `/work` / `/code` commands and strong-Cat Work/Code proposals across every Chat/Code/Work preset; SPEC-106 added: Chat Optimistic Message Identity and Refresh-Race Resilience for the workspace composer flicker fix; SPEC-105 revised: no-slash Work/Code intake now uses strong-Cat proposal tools with owner confirmation instead of default platform heuristic detection; SPEC-104 added: Direct Chat Slash-Mode Work Intake, covering `/chat` / `/work` / `/code`, same-Cat clarification/follow-up, provider capability gating, and weak/unknown human gate; SPEC-103 added: Clowder/Cattery data model; SPEC-102 added: Lobby sidebar IA and entity routes.)*
