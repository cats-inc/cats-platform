...
*Last updated: 2026-05-09 (SPEC-076 updated: `artifact` landed as the second entity-subscription kind through PLAN-098; SPEC-108 added: Cats Code Live Preview Substrate defines supervised process, port lease, logs, and Artifact Canvas preview-origin rules; SPEC-107 added: Preset-Neutral Product Intent Intake generalizes explicit `/chat` / `/work` / `/code` commands and strong-Cat Work/Code proposals across every Chat/Code/Work preset; SPEC-106 added: Chat Optimistic Message Identity and Refresh-Race Resilience for the workspace composer flicker fix; SPEC-105 revised: no-slash Work/Code intake now uses strong-Cat proposal tools with owner confirmation instead of default platform heuristic detection; SPEC-104 added: Direct Chat Slash-Mode Work Intake, covering `/chat` / `/work` / `/code`, same-Cat clarification/follow-up, provider capability gating, and weak/unknown human gate; SPEC-103 added: Clowder/Cattery data model; SPEC-102 added: Lobby sidebar IA and entity routes.)*
