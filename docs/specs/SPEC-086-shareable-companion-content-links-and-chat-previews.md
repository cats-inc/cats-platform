...
4. The first copied/inserted serialized form shall be:
   ...
   3. path segment 1 (`version`) is a syntactically valid version token (else
      ...
      structural checks
   4. percent-decoding succeeds on all remaining segments (else
      `invalid: malformed_percent_encoding`)
   ...
   arbitrary prose. Because step 3 short-circuits on unsupported versions, a
   future v2 link with extra segments pasted into a v1 parser returns
   `unsupported_version`, not `bad_segment_count`.
