...
4. The first copied/inserted serialized form shall be:
   ...
   The parser shall apply checks in this fixed order:

   1. scheme is `cats` (else `invalid: wrong_scheme`)
   2. host is `companion` (else `invalid: wrong_host`)
   3. percent-decoding succeeds on every path segment (else
      `invalid: malformed_percent_encoding`). Percent-decoding is validated
      before any version semantics so that a malformed reference cannot be
      laundered as `unsupported_version`.
   4. path segment 1 (`version`) is a syntactically valid version token (else
      `invalid: malformed_version`); if the version is well-formed but not
      supported, return `unsupported_version` immediately and stop further
      structural checks
   5. exactly the expected number of path segments for that version (else
      `invalid: bad_segment_count`)
   6. target type is one of `post`, `photo`, `video`, `music`, `file` (else
      `invalid: unknown_target_type`)

   The parser shall resolve through product-owned metadata rather than parsing
   arbitrary prose. A future v2 link with extra segments pasted into a v1
   parser returns `unsupported_version` (step 4 short-circuits before step 5),
   but a malformed reference such as `cats://companion/v2/%ZZ/...` returns
   `invalid: malformed_percent_encoding` because percent-decoding (step 3) runs
   before the version check.
5. The v1 `scopeId` shall be the platform-host product data scope id. It is a
   UUIDv4 generated by the platform host (the cats-platform process that owns
   the durable product data root, including the desktop main process for
   local-first installs) the first time a durable Cats product data root is
   created. It is persisted next to that data root by the platform host and
   exposed to Chat, Work, Settings, and companion renderer surfaces through the
   shared app-shell record (the same payload that already carries cats, cat
   bindings, and channel state). It is not an auth account id, browser
...
7. The v1 implementation layer is the chat renderer/composer parser and
   ...
   added later, the desktop main process shall own the handler and route only
   into the matching `scopeId`.
8. A `cats://companion/...` reference returned as `unsupported_version` shall
   not be treated as `missing` or `inaccessible`. The parser shall preserve the
   raw text and the UI shall render the affordance as follows:

   - **Composer (paste/insert)**: an inline subdued chip with the literal raw
     text and label `Unsupported version`; the chip is non-blocking — the user
     can leave it, edit it, or remove it before send.
   - **Transcript**: a fallback preview card visually matching the
     missing/deleted/inaccessible card, labelled `Unsupported reference
     version` plus the parsed version token; no `Open` action.
   - **Inspector / other surfaces**: plain text with a tooltip explaining the
     unsupported version. No surface shall throw, drop the message, or silently
     replace the raw text.

   Extra path segments inside the v1 form continue to return
   `invalid: bad_segment_count` per step 5 above.
...
type CompanionReferenceParseResult =
  ...
  | { status: 'invalid'; rawText: string; reason: CompanionReferenceParseInvalidReason };
