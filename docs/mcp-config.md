...
`mcpProfile` is a product-owned selector on Cats, channels, and the global
orchestrator, not an open provider flag. The current supported IDs live in
`src/shared/catMcpProfiles.ts`:
...
`POST /api/cats`, `PATCH /api/cats/{catId}`, `POST /api/channels`, and
global orchestrator updates reject unsupported `mcpProfile` IDs with
`400 bad_request`. Add new product profiles through the shared registry and the
corresponding product-owned resolver rather than passing arbitrary strings
through the API.
