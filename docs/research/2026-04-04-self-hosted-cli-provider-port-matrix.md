...
| Windows WSL substrate + first in-distro provider | `platform/windows/Install-WSL*`, `shared/windows/Install-WSLCLITools.ps1` | `scripts/windows/Install-WslUbuntuEnvironment.ps1`, `Install-WSLCLITools.ps1`, `Check-CLITools.ps1` (`Install-KiroWslCli.ps1` was later removed when Kiro moved onto the generic WSL provider loop) | Ported |
