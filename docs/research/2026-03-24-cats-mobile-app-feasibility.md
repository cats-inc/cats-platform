...
- 但可以共用：types（`@cats-inc/cats-platform/core`）、API client、business logic
...
cats-mobile/
  ...
  src/
    api/
      client.ts                 ← API client（共用 @cats-inc/cats-platform/core types）
...
```
@cats-inc/cats-platform          ← host（不變）
...
// @cats-inc/mobile/package.json
{
  ...
  "devDependencies": {
    "@cats-inc/cats-platform": "^1.0.0"    // 只用 type imports
  }
}
```
...
```typescript
import type { CoreTaskRecord, CoreApprovalStatus } from '@cats-inc/cats-platform/core';
```

### 共用的 types 自然來自 `@cats-inc/cats-platform/core`
...
- **跟 plugin 架構互不衝突**：mobile app 是 API consumer，不是 plugin，
  自然消費 `@cats-inc/cats-platform/core` 的 types
