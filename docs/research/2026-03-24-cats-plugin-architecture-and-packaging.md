...
- **Related**:
  - [ADR-025](../decisions/025-make-cats-inc-a-suite-host-with-core-owned-product-projections.md) — suite host 結構
  ...
  - [ADR-037](../decisions/037-serve-runtime-dashboard-and-playground-from-suite-host.md) — runtime tool hosting
...
- `design/` 是共享視覺語言
- `app/` 是 suite shell，擁有 product switcher 和 sidebar slot
...
@cats-inc/cats-platform/
  src/
    ...
    app/               ← SuiteShell, plugin loader, product switcher
...
```typescript
// app/renderer/SuiteShell.tsx
function SuiteShell({ plugins }: { plugins: CatsPlugin[] }) {
  const [activeId, setActiveId] = useState(defaultPluginId);
  const active = plugins.find(p => p.id === activeId);

  return (
    <div className="suiteShell">
      <Sidebar>
...
- **Phase 5 完成**：design 提取到 suite level — 否則 plugin 沒有共享 design tokens
