...
| `PhaseScopedWorkToolSurface` | `tool_call` | Product delegates and Chat bounded-observation descriptors implemented; runtime MCP exposure pending | Strong Cat / Boss Cat via phase-scoped observations | Cats Work supervised tool boundary | `src/products/work/shared/workToolSurface.ts`, `src/products/work/shared/workToolObservation.ts`, `src/products/work/state/workIntakeDelegate.ts` | [tool-calls.md](./tool-calls.md#phase-scoped-work-tools) |
