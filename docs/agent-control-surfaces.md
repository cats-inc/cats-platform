...
Conversation/membership/work mutations remain unregistered K3 responsibilities.
...
| `ProviderAgentToolFeedback` / `toolResults` | `bounded_observation` | K2 same-session feedback; at most four results / 24,000 serialized characters | Supervised Chat read loop | Coordinating provider-agent decision session | `src/platform/orchestration/providerAgentAdapter.ts` | [Orchestrator Collaboration Preparation](tool-calls.md#orchestrator-collaboration-preparation) |
| `CollaborationReport` | `finalization_envelope` | K2 preparation/status and receipts; revision/source revalidated at publication | Chat preparation loop | Chat transcript/dispatch merge | `src/products/chat/state/orchestratorCollaborationReport.ts` | [SPEC-118 K2](specs/SPEC-118-orchestrator-knowledge-and-collaboration-operations.md#k2-read-operation-contract) |
