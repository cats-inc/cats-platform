...
| 2026-04-28 | Reverted the `not-determined` passthrough: `desktopCapturer.getSources` does not reliably trigger the macOS Screen Recording TCC prompt, so passing through silently produced degraded screenshots with no toast and no consent dialog. `not-determined` is now mapped to `permission_denied` again so the renderer surfaces the toast warning, matching the original UX. |
