...
| `.env.example` | Modify | Document the `CATS_CHAT_NATURAL_PRODUCT_INTENT_MODE` deployment gate, its three values (`off` / `cat_tool` / `heuristic_prefilter`), and the effective-mode interaction with the owner-profile setting. |
...
| Date | Update |
|------|--------|
| 2026-05-07 | `.env.example` audit follow-up: full sweep of `src/**` env-var references against `.env.example` revealed 14 additional gaps unrelated to PLAN-094 itself (runtime session/idle/setup-proxy timeouts, provider-agent decision flow gate, provider capability bootstrap path override, Telegram bot/webhook trio, server startup orchestration trio, platform package root). All now documented inline near their feature areas with default-value notes. The post-fix audit (`grep src -r` vs `.env.example`) is clean for real env vars; only internal constants such as `CATS_CORE_STATE_VERSION` and the already-aliased `CATS_INC_HOST`/`CATS_INC_PORT` remain outside the template. |
| 2026-05-07 | Documentation gap fix: `CATS_CHAT_NATURAL_PRODUCT_INTENT_MODE` is now documented in `.env.example` with the three valid values, default `off`, the effective-mode interaction with the owner-profile setting, and the `heuristic_prefilter` detector-only caveat. Phase 1 landed the env var in `src/config.ts` but missed `.env.example`, so operators reading the env template had no way to discover the gate. |
