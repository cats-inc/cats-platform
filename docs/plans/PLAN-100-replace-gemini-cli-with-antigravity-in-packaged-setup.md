...
- [x] Identify whether `agy` creates a `.antigravity/skills/` directory or any equivalent skills mechanism. Answered from official docs, then confirmed by probe on 2026-08-28 (agy 1.1.20): two seeded project-level skill directories, `.gemini/skills/` and `.antigravity/skills/`, were both invisible to a print-mode turn that enumerated agy's in-context skills. Cats skills sync therefore drops Gemini without adding an Antigravity row. The probe corrects the observed roots — `~/.gemini/antigravity-cli/builtin/skills/` and `~/.gemini/config/plugins/<plugin>/skills/`, not `~/.gemini/antigravity-cli/plugins`. See SPEC-110 open questions for the full method.
...
- [x] In `scripts/windows/Sync-AgentSkills.ps1:16,35,67`:
  ...
  - No `antigravity` entry: Phase 0, confirmed by direct probe on 2026-08-28, found no project-level skills directory for `agy`. The agent list stays `claude` and `codex`.
