...
3. Do not invent `.antigravity/skills/` behavior — Phase 0 confirmed `agy` does not read it. Drop the Gemini row from skills sync without adding an Antigravity row: `agy` reads `.agents/skills/`, which the existing `codex` target already writes.
...
- [x] Identify whether `agy` creates a `.antigravity/skills/` directory or any equivalent skills mechanism. Probed 2026-08-28 in two passes. The first seeded `.gemini/skills/` and `.antigravity/skills/`; neither was visible, which was over-read as agy having no project-level skills at all. The second pass seeded `<workspace>/.agents/skills/` and agy reported that skill by absolute path, while a `.gemini/skills/` control stayed invisible. **`agy` reads project-level skills from `.agents/skills/`** — the same directory the existing `codex` sync target writes — and neither `.antigravity/skills/` nor `.gemini/skills/` is read. Cats skills sync therefore drops Gemini and needs no Antigravity row, because that destination is already covered. The probe also corrects the user-level roots to `~/.gemini/antigravity-cli/builtin/skills/` and `~/.gemini/config/plugins/<plugin>/skills/`, not `~/.gemini/antigravity-cli/plugins`. See SPEC-110 open questions for the full method.
...
- [x] Create `scripts/macos/install-antigravity.sh`. Implementation: publish the bash-style lifecycle contract (`--check`, `--apply`, `--upgrade`, `--force`, `--uninstall`, `--dry-run`, `--json`) and accept the PowerShell-style bridge aliases (`-CheckOnly`, `-Apply`, `-Upgrade`, `-Force`, `-Uninstall`, `-DryRun`, `-Json`) for packaged setup compatibility. Parse flags with a custom `while [ $# -gt 0 ]; do case "$1" in ... esac; shift; done` loop; do not use `getopts`, because compatibility aliases include single-dash long flags. Invoke the official Google installer for install / refresh actions and port environment-bootstrap's refresh behavior by deleting `~/.local/bin/agy` before upgrade / force.
...
- [x] In `scripts/windows/Sync-AgentSkills.ps1:16,35,67`:
  ...
  - No `antigravity` entry is required: Phase 0, corrected by a second probe pass on 2026-08-28, found `agy` reads project-level skills from `.agents/skills/` — the directory the existing `codex` target already writes. The agent list stays `claude` and `codex`; an alias for the same destination would be cosmetic.
