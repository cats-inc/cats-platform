...
| 2026-03-26 | Core-owned memory-maintenance action route landed: `POST /api/core/memory-maintenance` can now manually replay companion or owner canonical sync through the same Cats-owned maintenance seam and persist normalized maintenance outcomes back into Core activity history |
