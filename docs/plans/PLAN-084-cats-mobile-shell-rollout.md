...
| 2026-05-05 | IA reshape (`e221fa9c1`): bottom-tab inventory becomes `Cats / Chat / Code / Work / Settings` (Cats is English in zh-TW too). Mobile Lobby tab is replaced by a `Cats` tab whose content is the directory landing — three sections (My Cats / My Clowders / My Catteries with cat drill-down). Web Lobby content (greeting, entity-index cards, Guide Cat greeting) is explicitly NOT mirrored on mobile. The MY-lens row is removed from Chat / Code / Work product sidebars; cat / clowder / cattery rosters now live exclusively under the `Cats` tab. en primary actions ship Title Case (`+ New Chat / + Group Chat / + Parallel Chat`, etc.); zh-TW unchanged. Boundary contract trimmed: `selectMobileLobby` → `selectMobileCatsDirectory`, `MobileLobbyCopy` → `MobileCatsTabCopy`; `MobileSidebarCat / selectMobileMyCatsLens / selectMobileChatSidebar / MobileChatSidebarData` removed. ADR-092 and SPEC-095 carry "Subsequent Revision (2026-05-05)" sections capturing the reshape. |
| 2026-05-05 | Follow-up (`e40685c7c`): Work `+ Parallel Work` was silently falling through to `entryKind: 'default'` (mobile create contract has no parallel path). Mirrored Chat's parallel-only intercept and added `parallelWorkDesktopOnly{Title,Body}` copy keys (en + zh-TW). Dropped remaining dead cat/presence contracts (`MobileSidebarCatStatus`, `catStatusLabels`, `statusLabel`). Updated `architecture.md`, `requirements.md` (FR-025), `AGENT-GUIDE.md`, and `SPEC-102`'s mobile bottom-tab Non-Goal to match. |
