...
- [ ] Freeze the minimum CLI command surface for the first release:
      ...
      - optional `init`
      - optional `doctor`
- [ ] Freeze how `cats` discovers or creates local config, state, and data
      directories.
...
- [ ] Define shutdown ordering so `cats` can stop a managed runtime it
      started without affecting externally managed runtime instances.
- [ ] Define a minimal version-compatibility policy between `cats` and
      `cats-runtime`.
...
|------|--------|------------|
| The root package drifts back toward a library-shaped public API | High | Enforce ADR-013 constraints in implementation and review |
...
> Implement PLAN-013. Make `cats` runnable as an executable self-hosted npm
