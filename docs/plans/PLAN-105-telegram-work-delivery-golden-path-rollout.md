...
- [x] Add idempotent delivery primitives for the first modes and preserve the
      ...
      *`commit_only` and `artifact_only` are wired through the runtime delivery API;
      push/PR/preview keep their existing action mapping and remain unwired.*
...
- Polling still awaits the bridge per update (FR-14); one long request can still delay
  unrelated updates on the same binding.
- The permission-envelope check in the host resolver is a placeholder keyed on whether a
