...
| 2026-05-09 | Phase 5 verification slice landed: `src/core/linkageDiagnostics.ts` introduces `buildCoreLinkageDiagnostics(core)` aggregating mission, run, and transport-binding diagnostics into one `CoreLinkageDiagnosticsReport` (with summary counts and per-family arrays preserved) plus `isCoreLinkageHealthy(report)` for one-line gating. Audit / replay-verifier surfaces can now ask "what is broken across this canonical state?" with one call. Backed by `tests/linkage-diagnostics.test.ts`. |
