...
| 2026-03-29 | Phase 2 landed: `electron/contracts.ts` and `electron/packaging.ts` now expose a machine-readable `installer.providerSetup` contract covering setup modes, capability packs, source-knowledge boundaries, and the initial packaged-helper port queue, with desktop packaging tests locking that shape into the staged installer manifest |
...
| 2026-03-30 | Phase 3 slice 7 landed: `scripts/windows/Install-KiroWslCli.ps1` was introduced as a repo-owned packaged setup helper for the first WSL-backed provider installer; this was later superseded when Kiro moved onto the generic `Install-WSLCLITools.ps1` provider loop and the dedicated helper was removed |
...
| 2026-04-04 | Packaging slice 22 landed: repo-owned Unix unpacked-package smoke scripts now validate bundled sidecars, packaged setup assets, and platform-scoped installer contract truth for macOS/Linux outputs, filling the validation gap between Windows installed-app smoke coverage and the newly wired Unix package build commands |
