...
| 2026-04-28 | Implementation slice 17: removed Telegram bridge dependency on `buildOrchestratorTurnPlan`; Telegram-origin messages now enter the same Chat dispatch routing path directly, with a boundary test preventing the old planner import from returning. |
| 2026-04-28 | Implementation slice 18: removed parallel chat member dispatch dependency on `buildOrchestratorTurnPlan`; member summaries now derive from Chat dispatch prepared-turn state and provider-agent observations. |
