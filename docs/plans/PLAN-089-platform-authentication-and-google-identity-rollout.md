...
| 2026-05-10 | Phase 2 repair first-admin route slice started: `/api/auth/repair/first-admin` can recreate missing/corrupt auth state from an allowlisted loopback browser request, uses the loopback-or-token authorization helper, and is registered as a pre-auth public auth route. Startup token-state wiring and non-loopback token E2E coverage remain pending before Tasks 2.4/2.13 are checked off. |
