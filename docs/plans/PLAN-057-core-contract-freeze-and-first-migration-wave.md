...
| 2026-05-09 | Phase 5 first slice landed: `src/core/missionsByConversation.ts` introduces `buildMissionsByConversation(core)` returning a per-conversation index of missions with their runs folded in via three bridges (`mission.metadata.runId`, `run.metadata.missionId`, and a loose `conversationId === conversationId` fallback only when neither side explicitly references the other). Unanchored missions surface separately. `findMissionsForConversation(index, conversationId)` is the O(1) lookup. Closes the PLAN-057 §FR "conversation -> mission -> run" provenance coverage requirement. Backed by `tests/missions-by-conversation.test.ts`. |
