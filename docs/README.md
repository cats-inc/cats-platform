...
| [plans/](./plans/) | Complete | Implementation plans, including suite-foundation work, self-hosted npm app packaging, route-driven navigation, setup-wizard refinement, packaged setup/provider-installation knowledge porting, parallel-workstream seams, the chat sleep/wake lifecycle plan, the first dynamic room-workflow orchestration plan, the `Cats Chat v1` priority stack, the first `Cats Work` team-template intake flow, the first `Cats Code` local builder loop, and the new `cats` -> `cats-platform` host-rename migration |
