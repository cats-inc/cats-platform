...
export default function CodeRecentsScreen() {
  ...
  const { state } = useProductRecents('code');
    ...
    <SafeAreaView style={styles.container}>
      ...
      {state.kind === 'loading' ? (
        ...
        <Panel
          ...
          body="Set the desktop base URL in Settings to load Code recents."
        />
      ) : state.kind === 'error' ? (
        ...
        <Panel
          ...
          body="Recent Code conversations will appear here once you start one."
        />
      ) : (
        <FlatList
          data={state.recents}
          ...
          renderItem={({ item }) => (
            <Pressable
              onPress={() => router.push(`/(tabs)/code/${item.id}`)}
              style={({ pressed }) => [
                styles.row,
