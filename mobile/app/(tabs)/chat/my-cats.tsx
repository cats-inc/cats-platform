...
export default function ChatMyCatsScreen() {
  const router = useRouter();
  const { state } = useMyCatsLens('chat');

  return (
    <SafeAreaView style={styles.container}>
      <Stack.Screen options={{ title: 'My Cats', headerShown: true }} />
      {state.kind === 'loading' ? (
        ...
        <Panel
          ...
          body="Set the desktop base URL in Settings to load your cats."
        />
      ) : state.kind === 'error' ? (
        ...
        <Panel
          title="No cats yet"
          body="Create a cat assigned to the Chat product on the desktop."
        />
      ) : (
        <FlatList
          data={state.cats}
          ...
          renderItem={({ item }) => (
            <Pressable
              onPress={() => router.push(`/(tabs)/chat/direct-${item.id}`)}
              style={({ pressed }) => [
                styles.row,
