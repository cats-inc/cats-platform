...
function buildRuntimeProxyInjectionScript(): string {
  ...
  return [
    ...
    "    if (typeof Request === 'function' && input instanceof Request) source = input.headers;",
    '    if (!source) return [];',
    ...
    '      requestHeaders.set(csrfHeaderName, token);',
    '      return { input: new Request(input, { headers: requestHeaders }), init: init };',
    '    }',
    '    var headers = new Headers(init && init.headers);',
    '    headers.set(csrfHeaderName, token);',
    '    return { input: input, init: Object.assign({}, init || {}, { headers: headers }) };',
    '  }',
    '  function readCsrfToken(nativeFetch) {',
