...
import { cloneSnapshot, createEmptySnapshot, isoAt, normalizeSnapshot } from './snapshot.js';
import type {
  CompanionBoxStore,
...
export class MemoryCompanionBoxStore implements CompanionBoxStore {
  ...
  async upsertDerived(
    ...
    const nowIso = isoAt(now);
    const { box } = ensureCompanionBox(this.snapshot, catId, nowIso);
    const stored = upsertCompanionBoxDerived(this.snapshot, box, record, nowIso);
  ...
  async deleteMemory(
    ...
    const nowIso = isoAt(now);
    const { box } = ensureCompanionBox(this.snapshot, catId, nowIso);
    return deleteCompanionBoxMemory(this.snapshot, box, memoryId, nowIso);
  }
  ...
  async updateMemoryStatus(
    ...
    return structuredClone(
      updateCompanionBoxMemoryStatus(this.snapshot, box, memoryId, status, nowIso),
    );
  }
