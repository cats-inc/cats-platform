...
export async function resumeWorkflowContinuationReplay(input: {
  ...
  activeTurn.targetStatuses = preseededTargets.map(({ target, targetStateId }, targetIndex) => ({
    ...
    sourceMessageId: sourceMessage.id,
    sourceTurnId: input.request.sourceTurnId ?? null,
    sourceLaneId: input.request.sourceLaneId ?? null,
    sourceAssistantTurnId: input.request.sourceAssistantTurnId ?? null,
