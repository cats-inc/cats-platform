...
export function createWorkspaceProductApp({
  ...
  return function WorkspaceProductApp() {
    ...
    const onQuickAddDraftTemporaryParticipant = useCallback(() => {
      ...
      // synchronously. Build the next participant up-front so its id is
      // available when we queue the audience-keys update on the next line.
      ...
      const nextParticipant =
        draftTemporaryParticipants.length === 0 && participantCatIds.length === 0
          ? createDraftTemporaryParticipant({
